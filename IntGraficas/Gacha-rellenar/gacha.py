...
class Ventana(QWidget):
    # AGREGAR: señal para enviar una ruta
    ...
    def init_gui(self):
        ...
        self.setWindowTitle("Ejempló gacha")
        self.setStyleSheet("background-color: lightblue;")
        self.label_gacha = QLabel(self)
        self.label_gacha.resize(180, 200)
        
        ruta_images = os.path.join("images", "ball_machine.png")
        pixeles = QPixmap(ruta_images)
        self.label_gacha.setPixmap(pixeles)
        self.label_gacha.setScaledContents(True)

        self.boton_gacha = QPushButton("Obtener premio", self)
        self.boton_gacha.sizeHint()
        self.boton_gacha.clicked.connect(self.agregar_imagen)
